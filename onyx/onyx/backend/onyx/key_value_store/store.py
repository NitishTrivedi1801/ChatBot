...
def _convert_memoryview_for_json(obj: Any) -> Any:
    """Convert memoryview objects to bytes to ensure JSON serializability."""
    if isinstance(obj, memoryview):
        return obj.tobytes()
    elif isinstance(obj, dict):
        return {k: _convert_memoryview_for_json(v) for k, v in obj.items()}
    elif isinstance(obj, list):
        return [_convert_memoryview_for_json(item) for item in obj]
    else:
        return obj


class PgRedisKVStore(KeyValueStore):
    def __init__(self, redis_client: redis.Redis | None = None) -> None:  # type: ignore
    ...
    def store(self, key: str, val: JSON_ro, encrypt: bool = False) -> None:
        ...
        try:
            # Convert any memoryview objects to bytes before JSON serialization
            converted_val = _convert_memoryview_for_json(val)
            self.redis_client.set(
                REDIS_KEY_PREFIX + key, json.dumps(converted_val), ex=KV_REDIS_KEY_EXPIRATION
            )
        except Exception as e:
            # Fallback gracefully to Postgres if Redis fails
            logger.error(f"Failed to set value in Redis for key '{key}': {str(e)}")

        # Always serialize to JSON for Postgres storage, including strings
        serialized_val = json.dumps(val)
        encrypted_val = serialized_val if encrypt else None
        plain_val = serialized_val if not encrypt else None
    ...
    def load(self, key: str) -> JSON_ro:
        ...
        with get_session_context_manager() as db_session:
            ...
            try:
                # Convert any memoryview objects to bytes before JSON serialization
                converted_value = _convert_memoryview_for_json(value)
                self.redis_client.set(REDIS_KEY_PREFIX + key, json.dumps(converted_value))
            except Exception as e:
                logger.error(f"Failed to set value in Redis for key '{key}': {str(e)}")
            ...
            if value is None:
                if value is None:
                    raise KvKeyNotFoundError
                # Always deserialize from JSON before returning
                if not isinstance(value, str):
                    value = str(value)
                return cast(JSON_ro, json.loads(value))
