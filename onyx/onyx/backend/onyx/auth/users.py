    ...
    async def create(
        ...
        try:
            async with get_async_session_context_manager(tenant_id) as db_session:
                ...
                user_dict = (
                    user_create.dict() if hasattr(user_create, "dict") else dict(user_create)
                )
                user_dict["role"] = role

                try:
                    user = await super().create(user_dict, safe=safe, request=request)  # type: ignore
        ...
        #         user_dict = user_create.dict() if hasattr(user_create, "dict") else dict(user_create)
        #         user_dict["role"] = role
...
def get_oauth_router(
    ...
    async def callback(
        ...
        try:
            ...
            # Enhanced error logging for debugging Google OAuth failures
            response = getattr(e, 'response', None)
            status_code = getattr(response, 'status_code', None)
            body = getattr(response, 'text', None)
            logger.error(f"Google userinfo fetch failed: {e}, status={status_code}, body={body}")
            raise
