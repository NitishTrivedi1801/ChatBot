...
def get_inference_server_url_from_env() -> tuple[str, int]:
    """Get the inference model server host/port from GEN_AI_INFERENCE_SERVER env variable, or fallback to MODEL_SERVER_HOST:MODEL_SERVER_PORT."""
    url = os.environ.get("GEN_AI_INFERENCE_SERVER")
    if url:
        from urllib.parse import urlparse
        parsed = urlparse(url)
        host = f"{parsed.scheme}://{parsed.hostname}" if parsed.scheme and parsed.hostname else (parsed.hostname or "inference_model_server")
        port = parsed.port or 8001
        return host, port
    return str(MODEL_SERVER_HOST), int(MODEL_SERVER_PORT)
...
class EmbeddingModel:
    def __init__(
        ...
        # Use GEN_AI_INFERENCE_SERVER if not indexing and no explicit host/port
        if not server_host and not server_port and not (provider_type and getattr(provider_type, 'name', None) == 'INDEXING'):
            host, port = get_inference_server_url_from_env()
            model_server_url = build_model_server_url(host, port)
        else:
            model_server_url = build_model_server_url(server_host or MODEL_SERVER_HOST, server_port or MODEL_SERVER_PORT)
...
class RerankingModel:
    def __init__(
        ...
        api_url: str | None,
        model_server_host: str = "",
        model_server_port: int = 0,
    ) -> None:
        if not model_server_host and not model_server_port:
            host, port = get_inference_server_url_from_env()
            model_server_url = build_model_server_url(host, port)
        else:
            model_server_url = build_model_server_url(model_server_host or MODEL_SERVER_HOST, model_server_port or MODEL_SERVER_PORT)
