...
from onyx.document_index.vespa_constants import VESPA_APP_CONTAINER_URL, VESPA_APPLICATION_ENDPOINT
...
def get_vespa_http_client(no_timeout: bool = False, http2: bool = True) -> httpx.Client:  # type: ignore
    """
    Configure and return an HTTP client for communicating with Vespa,
    including authentication if needed.
    """

    return httpx.Client(  # type: ignore
        cert=(
...
def get_current_active_vespa_session() -> str | None:
    """
    Fetches the current active Vespa session ID from the Vespa /active endpoint.
    Returns the session ID as a string, or None if not found or on error.
    """
    url = f"{VESPA_APPLICATION_ENDPOINT}/tenant/default/application/default/environment/dev/region/default/active"
    try:
        with get_vespa_http_client() as client:
            response = client.get(url)
            response.raise_for_status()
            data = response.json()
            return str(data.get("session")) if "session" in data else None
    except Exception as e:
        logger.warning(f"Failed to fetch current active Vespa session: {e}")
        return None
