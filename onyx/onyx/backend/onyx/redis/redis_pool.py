...
from redis.lock import Lock as RedisLock  # type: ignore
...
class TenantRedis(redis.Redis):  # type: ignore
    ...
    def _convert_memoryview_to_bytes(self, value: Any) -> Any:
        """Convert memoryview objects to bytes to ensure JSON serializability."""
    ...
    def _prefix_method(self, method: Callable) -> Callable:
        ...
        def wrapper(*args: Any, **kwargs: Any) -> Any:
            # Convert any memoryview values to bytes
            if "value" in kwargs:
                kwargs["value"] = self._convert_memoryview_to_bytes(kwargs["value"])
            ...
            # Convert any memoryview values in args
            args = tuple(self._convert_memoryview_to_bytes(arg) for arg in args)
    ...
    def get(self, name: str | bytes | memoryview, *args: Any, **kwargs: Any) -> Any:
        """Override get method to handle memoryview conversion."""
        result = super().get(self._prefixed(name), *args, **kwargs)
        return self._convert_memoryview_to_bytes(result)

    def hget(self, name: str | bytes | memoryview, key: str | bytes | memoryview, *args: Any, **kwargs: Any) -> Any:
        def to_bytes(val):
            if isinstance(val, memoryview):
                return val.tobytes()
            if isinstance(val, str):
                return val.encode()
            return val  # already bytes
        name_bytes = to_bytes(name)
        key_bytes = to_bytes(key)
        # Ensure _prefixed returns bytes
        prefixed_name = self._prefixed(name_bytes)
        if isinstance(prefixed_name, str):
            prefixed_name = prefixed_name.encode()
        result = super().hget(prefixed_name, key_bytes, *args, **kwargs)  # type: ignore
        return self._convert_memoryview_to_bytes(result)

    def getset(self, name: str | bytes | memoryview, value: Any, *args: Any, **kwargs: Any) -> Any:
        """Override getset method to handle memoryview conversion."""
        def to_bytes(val):
            if isinstance(val, memoryview):
                return val.tobytes()
            return val
        ...
        name = to_bytes(name)
        value = self._convert_memoryview_to_bytes(value)
        result = super().getset(self._prefixed(name), value, *args, **kwargs)
        return self._convert_memoryview_to_bytes(result)
        result = super().getset(self._prefixed(name), self._convert_memoryview_to_bytes(value), *args, **kwargs)
        return self._convert_memoryview_to_bytes(result)
...
class RedisPool:
    ...
    _pool: redis.ConnectionPool  # type: ignore
    _replica_pool: redis.ConnectionPool  # type: ignore
    ...
    def get_raw_replica_client(self) -> Redis:
        ...
        without tenant prefixing.
        """
        return Redis(connection_pool=self._replica_pool)
    ...
    def create_pool(
        ...
        ssl: bool = False,
    ) -> redis.ConnectionPool:  # type: ignore
        """We use ConnectionPool because it will block and wait for a connection
        ...
        if ssl:
            return redis.BlockingConnectionPool(  # type: ignore
                host=host,
                port=port,
                db=db,
                password=password,
                ...
                connection_class=redis.SSLConnection,  # type: ignore
