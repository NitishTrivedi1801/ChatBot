...
from sqlalchemy import Sequence  # type: ignore
...
from sqlalchemy.types import LargeBinary  # type: ignore
from sqlalchemy.types import TypeDecorator  # type: ignore
from sqlalchemy import PrimaryKeyConstraint  # type: ignore
from sqlalchemy.orm.exc import StaleDataError  # type: ignore
...
class KVStore(Base):
    ...
    _encrypted_value = Column("encrypted_value", String, nullable=True)
    ...
    def encrypted_value(self, v):
        import json
        if v is not None and not isinstance(v, str):
            self._encrypted_value = json.dumps(v)
        else:
            self._encrypted_value = v
...
def upsert_persona(session, persona_id):
    # ... your code ...
    try:
        session.query(Persona__Tool).filter_by(persona_id=persona_id).delete()
        session.commit()
    except StaleDataError:
        # No rows matched for deletion; this is fine, just continue
        session.rollback()  # optional: rollback if you want to clear the failed transaction
        # Optionally log: logger.info("No persona__tool rows to delete for persona_id=%s", persona_id)
