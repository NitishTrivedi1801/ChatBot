...
def get_schema_options() -> tuple[str, bool, bool, bool]:
    ...
    if MULTI_TENANT and schema_name == POSTGRES_DEFAULT_SCHEMA and not upgrade_all_tenants:
        raise ValueError(
...
def provide_iam_token_for_alembic(
    ...
    if USE_IAM_AUTH:
        ...
        token = get_iam_auth_token(host, str(port), user, region)
        cparams["password"] = token
        cparams["ssl"] = ssl_context

async def run_async_migrations() -> None:
    schema_name, create_schema, upgrade_all_tenants, continue_on_error = get_schema_options()

    SqlEngine.init_engine(pool_size=20, max_overflow=5)

    # Use the same connection string building logic as the rest of the app
    from onyx.db.engine import build_connection_string
    from onyx.configs.app_configs import POSTGRES_HOST, POSTGRES_PORT, POSTGRES_DB, POSTGRES_USER, POSTGRES_PASSWORD, USE_IAM_AUTH
    ...
    if upgrade_all_tenants:
        ...
        for i_tenant, schema in enumerate(tenant_schemas, start=1):
            logger.info(f"Migrating schema: index={i_tenant} num_tenants={len(tenant_schemas)} schema={schema}")
            try:
