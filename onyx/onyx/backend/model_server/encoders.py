...
class CloudEmbedding:
    ...
    async def _embed_vertex(
        ...
        tasks = [
            client.get_embeddings_async(list(batch), auto_truncate=True) for batch in batches
        ]
...
async def embed_text(
    ...
    if provider_type is not None:
        ...
        try:
            ...
            logger.error(f"Model: {model_name}, Texts: {prefixed_texts}")
            raise RuntimeError(f"Model encoding failed: {str(e)}")
